...
insert into veiculos (modelo, ano, capacidade_carga) values
('Furgão', 2020, 2.00),
('Caminhão', 2018, 10.00),
('Van', 2019, 3.00),
('Carro', 2021, 1.00),
...
insert into entregas (id_veiculo, data_entrega, cidade, peso_entrega) values
...
select * from bd_empresa_logistica.veiculos;
select * from bd_empresa_logistica.entregas;
...
SELECT cidade, SUM(peso_entrega) AS total_peso 
    FROM bd_empresa_logistica.entregas GROUP BY cidade HAVING total_peso > 15.00;
...
SELECT v.modelo, sum(e.peso_entrega) AS total_peso_entregas
    FROM bd_empresa_logistica.veiculos v JOIN bd_empresa_logistica.entregas e 
